...
func TestString(t *testing.T) {
	for i, test := range []struct {
		...
		{
			l := JsonLexer{Data: []byte(test.toParse)}
...
func TestNumber2(t *testing.T) {
	for i, test := range []struct {
		...
		{toParse: `"10.00"`, want: 10.0},
		{toParse: "10.0000", want: 10.0},
	} {
		_ = i
		l := JsonLexer{Data: []byte(test.toParse)}
		if test.want != l.Float64Any() {
			t.Fatal(l)
		}
	}
}
...
func TestFloatMixed(t *testing.T) {
	for i, test := range []struct {
		toParse   string
		want      string
		wantError bool
		...
		got := l.numberOrString()
		if got != test.want && !test.wantError {
			t.Errorf("[%d, %q] numberOrString() = %v; want %v", i, test.toParse, got, test.want)
		}
		...
		gotParsed, err := strconv.ParseFloat(got, 64)
		if err == nil && test.wantError {
			t.Errorf("[%d, %q] number() ok; want error", i, test.toParse)
		}
		...
		wantParsed, err := strconv.ParseFloat(test.want, 64)
		if err != nil {
			t.Error(err)
			continue
		}
		if gotParsed != wantParsed {
			t.Errorf("%f != %f", gotParsed, wantParsed)
		}
	}
}
