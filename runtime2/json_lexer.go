...
type jsonLexerTokenKind byte

const (
	tokenUndef  jsonLexerTokenKind = iota // No token.
...
type token struct {
	kind jsonLexerTokenKind // Type of a token.
...
func (r *JsonLexer) unescapeStringToken() (err error) {
...
func (r *JsonLexer) errParse(what string) {
	if r.fatalError == nil {
		var str string
...
func (r *JsonLexer) errSyntax() {
	r.errParse("syntax error")
}

func (r *JsonLexer) errInvalidToken(expected string) {
	if r.fatalError != nil {
		return
	}
...
func (r *JsonLexer) unsafeString(skipUnescape bool) (string, []byte) {
	if r.token.kind == tokenUndef && r.Ok() {
		r.FetchToken()
	}
...
func (r *JsonLexer) number() string {
	if r.token.kind == tokenUndef && r.Ok() {
		r.FetchToken()
	}
...
func (r *JsonLexer) numberOrString() string {
	if r.token.kind == tokenUndef && r.Ok() {
		r.FetchToken()
	}
	if !r.Ok() {
	...
	if r.token.kind == tokenNumber {
		ret := *(*string)(unsafe.Pointer(&r.token.byteValue))
		r.consume()
		return ret
	}
...
func (r *JsonLexer) addNonfatalError(err *LexerError) {
	if r.UseMultipleErrors {
